    ...
    if let Ok(entries) = fs::read_dir(&emu_dir) {
        for entry in entries.flatten() {
            if entry.path().is_dir() {
                // Only count as installed if the directory is non-empty
...
async fn install_emulator(emulator_id: String, app_handle: tauri::AppHandle) -> Result<String, String> {
    ...
    if emu.archive_type == "zip" {
        ...
        // For 7z, we try to use system 7z or just keep the archive
    ...
    if exe_found {
        ...
        // Still "installed" but warn about executable
        Ok(format!("{} files extracted. Note: the executable '{}' was not found automatically — you may need to configure it manually.", emu.name, emu.executable_name))
    }
}
...
fn extract_7z(archive_path: &PathBuf, install_dir: &PathBuf) -> Result<(), String> {
    // Try using 7z.exe from system PATH
    ...
    match result {
        Ok(output) => {
            if output.status.success() {
                ...
                Err(format!("7z extraction failed: {}. Install 7-Zip and add it to PATH to extract .7z archives.", stderr))
            }
        }
        Err(_) => {
            Err("7-Zip not found. Please install 7-Zip and add it to your PATH to extract .7z archives. Alternatively, download the ZIP version of this emulator manually.".to_string())
        }
    }
}
...
fn launch_emulator(emulator_id: String, rom_path: Option<String>) -> Result<String, String> {
    ...
    let exe_path = find_executable(&install_dir, &emu.executable_name)
        .ok_or_else(|| format!(
            "Executable '{}' not found in {:?}. Try reinstalling the emulator.",
            emu.executable_name,
            install_dir
        ))?;
